import { loadIdMap, resolveIdFields } from "@freeanima/portal-sdk/offline-id-map";
import {
  registerOfflineModule,
  registerOfflineModuleCap,
} from "@freeanima/portal-sdk/offline-module-registry";
import type { FlushOpOutcome, RpcModuleAdapter } from "@freeanima/portal-sdk/offline-module-types";
import {
  enqueueOutboxOp,
  listOutboxOps,
  removeOutboxOp,
  ...
  refreshAll: async (_scope) => {
    try {
      const subject_id = await (
        await import("@freeanima/portal-sdk/world-context.ts")
      ).getUserSubjectId();

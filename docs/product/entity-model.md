...
- **UI:** shell `/vault` (`@freeanima/features/vault`); Bitwarden 未加密 JSON 导入（`import_refs.bitwarden` 幂等）；bundled Chat 有独立主密码解锁。Habitat「数据维护」可解锁 Agent 库（User 库 SSOT → `vault.agentKey.provision`）。

        ...
        <FormFieldset bordered={false} className="gap-3">
          ...
          <FormField label={m.admin_entities_col_visibility()} className="text-xs">
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="world-private">{m.admin_entities_visibility_private()}</Label>
            </div>
          </FormField>
          {form.private ? (
            <FormField label={m.admin_entities_col_owner_subject()} className="text-xs">
              ...
              >
                <SelectTrigger size="sm" className="w-full">
                  <SelectValue placeholder={m.admin_entities_owner_subject_placeholder()} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none__">
                    {m.admin_entities_owner_subject_placeholder()}
                  </SelectItem>
                  {subjects.map((s) => (
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{m.admin_entities_worlds_desc()}</p>
        </div>
      ...
      {loading ? (
        ...
        <StatusAlert variant="info">{m.admin_entities_worlds_empty()}</StatusAlert>
      ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{m.admin_entities_col_summary()}</TableHead>
                  <TableHead>{m.admin_entities_col_visibility()}</TableHead>
                  <TableHead>{m.admin_entities_col_owner_subject()}</TableHead>

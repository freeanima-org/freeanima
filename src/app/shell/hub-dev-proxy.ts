...
/**
 * Vite `/hub` `/mcp` proxy 目标：
 * 1. `FREEANIMA_URL`
 * 2. `~/.anima/server.status.json`（dev:hub 写出的 port）
 * 3. 回退 `http://127.0.0.1:2658`
 */
export function resolveProxyHubUrl(env: NodeJS.ProcessEnv = process.env): {
  url: string;
  ...
  return { url: DEFAULT_PROXY_HUB_URL, source: "default" };
}
...
/** Vite server.proxy 条目：Hub REST + WS */
export function createHubDevProxyOptions(target: string): ProxyOptions {
  return {
    target,
    changeOrigin: true,
    ws: true,
...
export function createHubDevProxyMap(
  target: string,
  extraPaths: readonly string[] = [],
): Record<string, ProxyOptions> {
  const opts = createHubDevProxyOptions(target);

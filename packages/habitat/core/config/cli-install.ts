...
export const CLI_UPGRADE_HINT_SOURCE =
  "源码安装不支持自动 upgrade。请手动执行：git pull、bun install，然后重启服务（anima service restart）。开发用 bun packages/habitat/portal/cli/cli.ts 或 just dev；本机常驻请 just install cli。";
...
export function getCliInstallKind(scriptPath?: string): CliInstallKind {
  ...
  if (resolved.endsWith("/packages/habitat/portal/cli/cli.ts")) return "source";

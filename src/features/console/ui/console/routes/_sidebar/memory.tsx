...
function MemoryPage() {
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      setError(
        m.console_common_search_failed({
  ...
  return (
    <div>
      <MemoryBrowseTabs />
      <div className="mb-4">
        <h2 className="text-lg font-bold">{m.console_nav_memory()}</h2>
        <p className="text-sm text-muted-foreground mt-1">{m.console_memory_desc()}</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
        <Card className="bg-muted py-0">
          <CardContent className="py-3 px-4">
            <p className="text-xs text-muted-foreground">{m.console_dashboard_semantic_memory()}</p>
            <p className="text-xl font-mono mt-1">{semanticMemoryCount}</p>
            <p className="text-xs text-muted-foreground mt-1">
              {m.console_api_semantic_memory_count({ count: String(semanticMemoryCount) })}
            </p>
          </CardContent>
        </Card>
        <Card className="bg-muted py-0">
          <CardContent className="py-3 px-4">
            <p className="text-xs text-muted-foreground">
              {m.console_dashboard_dialogue_messages()}
            </p>
      ...
      <div className="mb-4">
        <p className="text-xs text-muted-foreground mb-1.5">{m.console_memory_quick_links()}</p>
        ...
        <Card className="bg-muted py-0">
          <CardContent className="gap-3 py-4 px-4">
            <FormFieldset bordered={false} className="gap-3">
              <FormField label={m.console_memory_query_required()} className="text-xs">
                <Input
                  value={query}
      ...
      {searched && !searching && isEmpty ? (
        <StatusAlert variant="info">{m.console_memory_not_found({ query: lastQuery })}</StatusAlert>
      ) : null}
          ...
          <section>
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <h3 className="text-sm font-bold">
                {m.console_memory_recall_results()}
              ...
              <div className="flex flex-wrap gap-1">
                ...
                >
                  {m.console_memory_type_filter_all()} {result.results.length}
                </Badge>
            ...
            <div className="space-y-2">
              {filteredResults.length === 0 ? (
                <p className="text-sm text-muted-foreground">{m.console_common_no_results()}</p>
              ) : (

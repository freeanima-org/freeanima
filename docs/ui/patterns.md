...
| 交互   | **pointer：** `ContextMenu`。**touch：** `ActionSheet` + 长按（聊天室用常驻归档代替 ⋯）。聊天室会话行：touch 常驻归档；pointer 仅 active/hover（及 focus-within）露出。状态机分开；会话数据共享（目标 + 条目） |
| 壳     | N/A                                                                                                                                                                                                            |
...
**实现：** [`QuickAddBar.tsx`](../../packages/ui-kit/composite/QuickAddBar.tsx)。
...
**实现：** [`PullToRefresh.tsx`](../../packages/ui-kit/composite/PullToRefresh.tsx)。产品说明 → [page-refresh.md](../aspects/page-refresh.md)。
...
**实现：** [`ListDetailLayout.tsx`](../../packages/ui-kit/layout/ListDetailLayout.tsx)、ThreeColumnLayout；chrome 助手 [`DetailPanelShell.tsx`](../../packages/ui-kit/composite/DetailPanelShell.tsx)、[`ModuleScopeBar.tsx`](../../packages/ui-kit/composite/ModuleScopeBar.tsx)。
...
**实现：** [`EmptyState.tsx`](../../packages/ui-kit/composite/EmptyState.tsx)、[`StatusAlert.tsx`](../../packages/ui-kit/composite/StatusAlert.tsx)。
...
**实现：** [`auto-persist-schedule.ts`](../../packages/ui-kit/lib/auto-persist-schedule.ts)；详情面板经 [`useDetailPanelState.ts`](../../packages/ui-kit/composite/useDetailPanelState.ts)（默认长文预设）。Toggle/Switch 与栖息地显式保存按钮保持立即——非 AutoPersist。

...
壳 `/calendar`：单行工具栏（视图菜单、日期导航、kinds、重复展开、刷新、新建）；固定用户 subject（不暴露 user/agent
切换）。
...
- **着色**：按条目 kind（事件 / 任务 / 项目）语义色区分，与议程徽章一致
- **工具栏开关本机持久化**（`freeanima.calendar.uiPrefs`）：重复展开、视图（日/近三天/近七天/周/月）、事件/任务/项目；**窄布局（手机）与宽布局（桌面）共用**同一偏好，再次进入保留上次选择

...
import { isCodingConversationMeta } from "@freeanima/core/tool";
...
import { registerSkillsCatalogSystemPromptHook } from "@freeanima/capabilities/tools/skills-prompt-hooks";
import { ANIMA_URI_PROTOCOL_BODY } from "@freeanima/capabilities/tools/anima-uri-prompt";
...
import { loadSelfLayerInner } from "@freeanima/capabilities/self";
...
export function registerTemporalSummarySystemPromptHook(ctx: Context): void {
  onSystemPromptBuild(ctx, async (event) => {
    ...
    try {
      ...
      const { cacheGetJson, cacheSetJson } = await import("@freeanima/core/redis");
      ...
      const { cstDaySourceRef, notifySoftFailure } = await import("@freeanima/core/soft-failure");

...
function returnKindLabel(kind: ToolsStatusToolItem["return_kind"]): string {
  return kind === "text" ? m.admin_tools_kind_text() : m.admin_tools_kind_json();
}

function returnKindHint(kind: ToolsStatusToolItem["return_kind"]): string {
  return kind === "text" ? m.admin_tools_return_text() : m.admin_tools_return_json();
}
...
async function copyText(text: string, label: string): Promise<void> {
  try {
    ...
    console.info(m.admin_common_copied({ label }));
  } catch {
    console.warn(m.admin_common_copy_failed({ label }));
  }
}
    ...
    <div>
      ...
      <div className="space-y-4">
        ...
        {ungroupedTools.length > 0 ? (
          <>
            <h3 className="text-sm font-bold mt-4 mb-2">{m.admin_tools_ungrouped()}</h3>
            <div className="space-y-3">

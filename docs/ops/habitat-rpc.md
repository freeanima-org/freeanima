...
实现于 [`packages/shared/habitat-rpc/`](../../packages/shared/habitat-rpc/)（WS 信封 + HTTP REST 辅助）与 [`packages/server/habitat/http-rest-router.ts`](../../packages/server/habitat/http-rest-router.ts)（HTTP 适配器）。功能方法 schema 在 [`packages/shared/rpc-contract/`](../../packages/shared/rpc-contract/)。
...
服务端：[`packages/capabilities/outpost/`](../../packages/capabilities/outpost/)。  

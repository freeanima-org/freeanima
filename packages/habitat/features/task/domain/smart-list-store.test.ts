...
import { SMART_LIST_COMPONENT } from "@freeanima/habitat/core/db/schema/entity";
...
describe("listSmartListsMerged", () => {
  ...
  test("仅合并自定义 entity（内置完成类 preset 已退役）", async () => {
    ...
    expect(rows.filter((r) => r.preset != null).length).toBe(0);

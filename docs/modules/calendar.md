...
- 议程行溢出菜单（pointer 右键 / touch 长按）：**事件** 编辑 · 开始番茄 · 转为任务 · 删除；**任务** 编辑 · 开始番茄（pending）· 标记完成/未完成 · 转为事件（有计划/截止时）· 删除；**项目** 打开；**节日** 无菜单（只读）
...
- **日视图**：单日议程，上一页/下一页按自然日切换（可到明天、后天）；「今天」跳回当日。议程列表分段：**逾期**（仅今天；截止日或计划结束日早于今天的 pending，含项目内）→ **安排**（事件 + 无项目 pending）→ **项目**（可折叠；其下嵌套该项目当日非逾期的 pending/已完成）→ **节日** → **已完成**（无项目归属）。同刻按优先级二级排序。已过期事件由本视图开关控制。
- **近三天 / 近七天**：相对真实今天的滚动议程（3 / 7 个自然日）；每个自然日同一套分段；逾期仅挂在真实今天
- **月 / 周 · 选中日议程**：与日视图同一套分段与项目嵌套；月格/周格条带布局不变

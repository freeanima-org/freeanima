...
| 概念            | 是什么                                                             | 代码                                                                             |
| --------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------------- |
| **壳（Shell）** | 入口运行时宿主（browser / Tauri；构建目标 web / desktop / mobile） | `packages/portal/app/*`；`portal-sdk`（`getShellKind`、`ShellApi`、buildTarget） |
| **应用布局**    | 模块 Rail / 底栏、设置 chrome                                      | `packages/app-frame`（`AppFrame`）；跟**视口**，不跟壳                           |

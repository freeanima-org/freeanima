...
function formatPlatforms(platforms?: string[]) {
  if (!platforms?.length) return m.console_common_all();
...
function CommandsPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-1">{m.console_nav_commands()}</h2>
      <p className="text-sm text-muted-foreground mb-4">{m.console_commands_desc()}</p>

      {conversationCommands.length > 0 ? (
        <section className="mb-6">
          <h3 className="text-sm font-semibold mb-2">{m.console_commands_conversation_scope()}</h3>
          <p className="text-xs text-muted-foreground mb-2">
            {m.console_commands_conversation_hint({
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-48">{m.console_commands_command()}</TableHead>
                  <TableHead>{m.console_commands_description()}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
      ...
      {globalCommands.length > 0 ? (
        <section>
          <h3 className="text-sm font-semibold mb-2">{m.console_commands_global_scope()}</h3>
          <p className="text-xs text-muted-foreground mb-2">
            {m.console_commands_global_hint({ count: String(globalCommands.length) })}
          </p>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-48">{m.console_commands_command()}</TableHead>
                  <TableHead>{m.console_commands_description()}</TableHead>
                  <TableHead className="w-40">{m.console_commands_platform()}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
      ...
      {conversationCommands.length === 0 && globalCommands.length === 0 ? (
        <StatusAlert variant="info">{m.console_commands_empty()}</StatusAlert>
      ) : null}
    </div>
  );
}

...
实现目标：`packages/features/project/`（domain + habitat + `plugin.ts`）与 `packages/ui-features/project/`（`ui/spa`）；schema 在 `packages/core/db/schema/entity/components/`；SAP frames 在 `packages/shared/rpc-contract/frames/`。

...
跨模块**时间视图 / 入口**：自有 **事件**（`calendar_event`）+ 带**计划时间**的任务（清单/backlog 与项目内，含重复虚拟展开）+ 项目 **start_at/end_at**，经 `calendar.range` 聚合到月/周/日视图。  
...
- **重复展开**开关：控制是否显示虚拟重复实例（月条、选中日议程、周列共用同一过滤；关闭时隐藏 `virtual: true`）

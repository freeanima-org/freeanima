...
- **Platform-native UX** — Mobile and desktop are **separate interaction and layout designs**, not one responsive skin stretched across form factors. Shared contracts (API, SAP, settings keys) may exist; presentation and interaction patterns should fit each platform. **Two-layer model:** shell/capability layer (Electron, Capacitor, companion, Hub wiring) is platform-native; **UI layer** is a shared SPA (`app/web/dist` on Hub `/web/*`) with **capability axis** (`satelliteShell`: storage, IPC, Keyboard) separate from **presentation axis** (`layoutMode`: `compact` bottom-nav vs `expanded` top-nav). `resolveLayoutMode()` drives nav/IA (phone browser, PWA standalone, Capacitor → `compact`; Electron / desktop browser → `expanded`). `detectPlatform()` is for settings sections and native capabilities only. Responsive CSS is allowed within the same `layoutMode` for fine-tuning (e.g. settings sidebar).

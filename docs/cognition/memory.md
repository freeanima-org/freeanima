...
| `retain`                                                               | 可重放抽取；默认 user/assistant **正文**为素材（无思考链/tool 角色）；自身经 AutoLlm 调 `memory_semantic_*`，过程仅见 `auto_llm_*`，约 20 字总结收尾；语义相关按本回合各条 user/assistant 分别 hybrid 召回后分配额合并 |

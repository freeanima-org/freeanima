...
function CronPage() {
  ...
  const onToggle = async (job: CronJob, enable: boolean) => {
    ...
    try {
      ...
      logCaughtError("routes/_sidebar/cron", e);
      setError(
        m.console_cron_toggle_failed({
          name: String(job.name ?? job.id),
          action: enable ? m.console_common_start() : m.console_common_stop(),
          detail: e instanceof Error ? e.message : String(e),
        }),
      );
    } finally {
  ...
  const runNow = async (job: CronJob) => {
    ...
    try {
      ...
      setToast((t) => ({
        ...t,
        [job.id]: (data as { message?: string }).message || m.console_cron_triggered(),
      }));
      setTimeout(() => {
        setToast((t) => {
          const next = { ...t };
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          <h2 className="text-lg font-bold">{m.console_nav_cron()}</h2>
          <p className="text-sm text-muted-foreground mt-1">
            {m.console_cron_desc()} <code className="text-xs">cronjob</code>
          </p>
        </div>
        <Button
          type="button"
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              [m.console_cron_total(), jobs.length],
              [m.console_cron_active(), activeCount],
              [m.console_cron_paused(), pausedCount],
            ].map(([label, value]) => (
          ...
          {jobs.length === 0 ? (
            <StatusAlert variant="info">{m.console_cron_empty()}</StatusAlert>
          ) : (
            jobs.map((job) => (
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Badge variant={job.paused ? "ghost" : "success"} className="text-xs">
                        {job.paused
                          ? m.console_cron_status_paused()
                          : m.console_cron_status_active()}
                      </Badge>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="flex items-center gap-2">
                        <Label htmlFor={`cron-enable-${job.id}`} className="text-xs">
                          {m.console_cron_enable()}
                        </Label>
                        <Switch

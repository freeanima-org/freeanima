...
/** 组合根在 Admin 启动前绑定进程 Logger（与 createServiceLogger 同一实例） */
export function bindAdminApiLogging(logger: Logger): void {
  apiLogger = logger.with({ component: "api" });
  sseLogger = logger.with({ component: "sse" });
}

export function resetAdminApiLoggingForTest(): void {
  apiLogger = null;
  sseLogger = null;
}

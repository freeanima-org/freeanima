import { advanceScheduleAt, type TaskRecurrence } from "@freeanima/host/core/db/schema/entity";
import { formatCstIso } from "@freeanima/host/core/util";

import type { CalendarRangeTaskItem } from "./types.ts";

const MAX_VIRTUAL = 64;

function dayKey(iso: string): string {
  const m = /^(\d{4}-\d{2}-\d{2})/.exec(iso);
  return m?.[1] ?? iso.slice(0, 10);
}

/**
 * 在 [from,to] 内虚拟展开重复任务后续实例（不写库）。
 * live 当期 due 已由 range-store 收录时跳过同日。
 */
export function expandRecurringTaskVirtuals(opts: {
  id: number;
  title: string;
  status: "pending" | "completed";
  project_id: number | null;
  list_id: number | null;
  due_at: string;
  recurrence: TaskRecurrence;
  from: string;
  to: string;
}): CalendarRangeTaskItem[] {
  const fromDay = dayKey(opts.from);
  const toDay = dayKey(opts.to);
  const out: CalendarRangeTaskItem[] = [];
  let cursor = opts.recurrence.schedule_at || opts.due_at;
  let guard = 0;
  while (guard < MAX_VIRTUAL) {
    guard += 1;
    let nextIso: string;
    try {
      nextIso = advanceScheduleAt(cursor, opts.recurrence);
    } catch {
      break;
    }
    const nextDay = dayKey(nextIso);
    if (nextDay > toDay) break;
    cursor = nextIso;
    if (nextDay < fromDay) continue;
    if (nextDay === dayKey(opts.due_at)) continue;
    out.push({
      kind: "task",
      id: opts.id,
      title: opts.title,
      due_at: formatCstIso(new Date(nextIso)),
      status: opts.status,
      project_id: opts.project_id,
      list_id: opts.list_id,
      virtual: true,
    });
  }
  return out;
}

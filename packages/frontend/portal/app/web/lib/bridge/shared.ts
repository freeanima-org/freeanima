import type { WebUiConfigJson } from "@freeanima/client/portal-sdk/web-ui-config";
...
export type ScopedSettingsBridge =
  import("@freeanima/client/portal-sdk/settings").ScopedSettingsBackend & {
    test(
      scope: import("@freeanima/client/portal-sdk/settings").SettingsStorageScope,
      value: unknown,
    ): Promise<unknown>;
  };
...
export async function fetchWebUiConfig(): Promise<WebUiConfigJson | null> {
  try {
    ...
    const { parseWebUiConfigJson } = await import("@freeanima/client/portal-sdk/web-ui-config");

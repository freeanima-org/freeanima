...
describePg("entity task PG", () => {
  ...
  it("creates task_list and task_item entities", async () => {
    ...
    const item = await createTaskItem({ title: "写文档", list_id: list.id });
    ...
    expect(asTaskItem(itemRow!)).toMatchObject({ title: "写文档", list_id: list.id });
  });
  ...
  it("lists and completes task items", async () => {
    ...
    const second = await createTaskItem({ title: "运动", list_id: list.id });

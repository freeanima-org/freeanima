...
Token 估算：`packages/core/src/token-estimate.ts`（与 `conversation-stats` 共用）。
...
| 模块 | 职责 |
|------|------|
| `packages/core/src/compressor.ts` | l 点、`deriveBoundariesFromL4`、`shouldAdvance`、`buildRuntimeFromLPoints`、`slimMessage` |
| `packages/core/src/compression-config.ts` | 配置与 `context_window` / 有效预算 |
| `packages/core/src/compression-summary.ts` | 摘要 LLM |
| `packages/core/src/compression-tool-loop.ts` | `isInToolLoop` |
| `packages/core/src/conversation.ts` | `recompressSession`、`buildRuntimeMessages`、`maybeApplyEmergencyCompression` |
| `packages/core/src/engine.ts` | emergency 调用点 |
| `packages/core/src/conversation-stats.ts` | `/stats` 展示 `l2`/`l3`/占用率 |

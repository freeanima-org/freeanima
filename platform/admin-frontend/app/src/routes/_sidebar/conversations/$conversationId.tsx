...
function ConversationDetailPage() {
  ...
  const acpDock = useAcpProgressDock(conversationId, {
    onDecision: async (sid) => {
      await useAdminConversationsStore.getState().selectConversation(sid, store.currentPage());
    },
  });

  useEffect(() => {
    void useAdminConversationsStore.getState().ensureConversationHeadline(conversationId);
  }, [conversationId]);

  useEffect(() => {
    void useAdminConversationsStore.getState().selectConversation(conversationId, 1);
  }, [conversationId]);

import { formatSkillsPrefix, skillPolicyFragments } from "@freeanima/core/skill";
import { resolveSubagentToolPolicy, materializeToolNames } from "@freeanima/core/capability-policy";
import { getProfileHopFormat, getProfileHopModel } from "@freeanima/core/config";
import {
  PROFILE_CHAT,
...
import { getToolConversationId, reportToolProgress, toolResult } from "@freeanima/core/tool";
import { composeAutoLlmPrompt } from "@freeanima/core/llm/auto-llm-prompt";
import { PROMPT_XML_TAGS } from "@freeanima/core/hooks/prompt";
import { resolveBoundAgentForConversation } from "@freeanima/engine/conversation/resolve-conversation-agent.ts";
  ...
  if (hasNamed) {
    ...
    if (!row) {
      const { resolveProjectAgentOverlay } =
        await import("@freeanima/core/skill/project-overlay.ts");
      const slug = task.slug?.trim().toLowerCase() ?? "";
      const hit = slug

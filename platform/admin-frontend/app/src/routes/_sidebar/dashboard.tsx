          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-stretch">
            ...
            <StatCard title={m.admin_common_tools()}>
              ...
              <p className="text-xs text-muted-foreground">
                {m.admin_dashboard_tools_registered()}
              </p>
            </StatCard>
            ...
            <StatCard title={m.admin_dashboard_slash_commands()}>
              ...
              <p className="text-xs text-muted-foreground">{m.admin_dashboard_all_platforms()}</p>
            </StatCard>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <StatCard title={m.admin_dashboard_dialogue_messages()}>
              ...
              <p className="text-xs text-muted-foreground">{m.admin_dashboard_messages_count()}</p>
            </StatCard>
          </div>
        </section>
      </div>
    ...
    <Card className="bg-muted py-0 lg:col-span-2">
      <CardContent className="py-3 px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-2">
          <div>
            <h4 className="text-sm text-muted-foreground">{m.admin_dashboard_service_status()}</h4>
          ...
          <div>
            <h4 className="text-sm text-muted-foreground">{m.admin_dashboard_uptime()}</h4>
            <p className="text-xl font-mono mt-1">{formatUptime(svc.uptime_seconds) || "—"}</p>
          </div>
          <div>
            <h4 className="text-sm text-muted-foreground">{m.admin_dashboard_process_memory()}</h4>
          ...
          <div>
            <h4 className="text-sm text-muted-foreground">{m.admin_dashboard_current_model()}</h4>
...
function TunnelLinksCard({ tunnel }: { tunnel: NonNullable<ServiceStatus["tunnel"]> }) {
  return (
    <Card className="bg-muted py-0 lg:col-span-3">
      <CardContent className="py-3 px-4">
        <h3 className="text-sm text-muted-foreground">{m.admin_dashboard_tunnel()}</h3>
        <div className="mt-2 flex flex-wrap gap-x-6 gap-y-2 text-sm">
          <div>
            <span className="text-muted-foreground">{m.admin_dashboard_tunnel_public()}: </span>
            <a
          ...
          <div>
            <span className="text-muted-foreground">{m.admin_dashboard_tunnel()}: </span>
            <a
              href={tunnel.api_url}
              className="text-primary underline-offset-4 hover:underline text-xs font-mono"
              target="_blank"
    ...
    <Card className="bg-muted py-0">
      <CardContent className="py-3 px-4">
        <h3 className="text-sm text-muted-foreground">
          {m.admin_dashboard_platform_connections()}
        </h3>
        {entries.length === 0 ? (
  ...
  return (
    <div className="group/session relative h-full">
      <StatCard title={m.admin_dashboard_conversations()}>
        ...
        <p className="text-xs text-muted-foreground">{m.admin_dashboard_hover_platforms()}</p>
      </StatCard>
      <div
        role="tooltip"

import { logStartupError } from "@freeanima/habitat/platform/logging";
import { loadBootstrapConfig } from "@freeanima/habitat/platform/config/bootstrap.ts";
import { parseBindHosts } from "@freeanima/habitat/platform";
import {
  resolveHabitatTlsListenConfig,
  toHabitatTlsBunOptions,
} from "@freeanima/habitat/platform/tls/resolve-habitat-tls";
import { startAcmeRenewalScheduler } from "@freeanima/habitat/platform/tls/acme";
...
export async function runServiceStack(options: ServiceStackOptions): Promise<void> {
  ...
  if (webEnabled) {
    try {
      ...
      if (distDir) {
        ...
        try {
          const { readAppVersion } = await import("@freeanima/habitat/core/config/version");
  ...
  const { serve } = await import("@freeanima/habitat/platform");

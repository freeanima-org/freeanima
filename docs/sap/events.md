...
After `message.send` returns `stream_id`, Hub pushes `stream.*` events. Defined in [`packages/sap-contract/src/frames/message.ts`](../../packages/sap-contract/src/frames/message.ts) as `streamEventMethods`.
...
Constants: `TERMINAL_EVENT_METHODS` in [`frames/terminal.ts`](../../packages/sap-contract/src/frames/terminal.ts).

...
- **React**（feature-habitat / ui-kit / satellites override）: `react/rules-of-hooks` error；`react/exhaustive-deps` warn。
...
**Flow**: change `src/core/db/schema/` → **`drizzle-kit generate`** → **`migrate`**.
...
Table shapes SSOT: [`src/core/db/schema/`](../../src/core/db/schema/). User ops (install, backup): [`docs/guide/database.md`](../../docs/guide/database.md).

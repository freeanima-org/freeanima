...
const pool = ["memory_semantic_search", "file_read", "browser_navigate", "notification_send"];
...
describe("toolNamesForInvisiblePolicy", () => {
  ...
  it("intersects pool with allowed tools", () => {
    expect(
      toolNamesForInvisiblePolicy(pool, policy(["memory_semantic_search", "file_read"])),
    ).toEqual(["memory_semantic_search", "file_read"]);
  });

  it("drops allows that are not in the pool", () => {
    expect(
      toolNamesForInvisiblePolicy(pool, policy(["memory_semantic_search", "unknown_tool"])),
    ).toEqual(["memory_semantic_search"]);
  });
});

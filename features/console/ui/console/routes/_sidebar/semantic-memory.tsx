...
import type { SemanticMemoryRow } from "@freeanima/console-contract/api";
import {
  Badge,
  Button,
  Card,
...
import { listSemanticMemories, updateSemanticMemoryPinned } from "@console/lib/api.ts";
...
function SemanticMemoryPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-1">{m.console_nav_semantic()}</h2>
      <p className="text-sm text-muted-foreground mb-4">{m.console_semantic_desc()}</p>
        ...
        <Card className="bg-muted py-0">
          <CardContent className="gap-3 py-4 px-4">
            <FormFieldset bordered={false} className="gap-3">
              <FormField label={m.console_semantic_search_fts()} className="text-xs">
                <Input
                  value={query}
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value={ALL_VALUE}>{m.console_common_all()}</SelectItem>
                ...
                <div>
                  ...
                  <Select value={statusFilter} onValueChange={setStatusFilter}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="all">{m.console_common_all()}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <FormFieldLabel className="text-xs py-0">
                    {m.console_semantic_source_conversation()}
                  </FormFieldLabel>
                  <Input
                  ...
                  <Select value={sortBy} onValueChange={(v) => setSortBy(v as BrowseSortBy)}>
                    ...
                    <SelectContent>
                      <SelectItem value="updated_at">
                        {m.console_semantic_sort_updated()}
                      </SelectItem>
                      <SelectItem value="created_at">
                        {m.console_semantic_sort_created()}
                      </SelectItem>
                      <SelectItem value="reference_count">
                        {m.console_semantic_sort_reference_count()}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
      ...
      {loaded ? (
        <div className="space-y-3">
          {loading && items.length === 0 ? (
            ...
            <StatusAlert variant="info">{m.console_common_no_results()}</StatusAlert>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{m.console_common_type_label()}</TableHead>
                    <TableHead>{m.console_common_status_label()}</TableHead>
                    <TableHead>{m.console_semantic_pinned()}</TableHead>
                    <TableHead>{m.console_semantic_created()}</TableHead>
                    <TableHead>{m.console_semantic_updated()}</TableHead>
                    <TableHead>{m.console_semantic_reference_count()}</TableHead>
                    <TableHead>{m.console_limbic_content()}</TableHead>
                    <TableHead>conversations</TableHead>
                    {hasSearchQuery ? <TableHead>{m.console_semantic_rank()}</TableHead> : null}
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {items.map((row) => (
        ...
        <p className="text-sm text-muted-foreground">{m.console_common_click_query_hint()}</p>
      )}
    </div>
  );
}

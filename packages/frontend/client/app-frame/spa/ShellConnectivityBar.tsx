...
function reconnectHabitatOnce(reconnectingRef: { current: boolean }): void {
...
export function ShellConnectivityBar(): null {
  ...
  // 勿把每次 render 新建的 notice 对象放进 effect deps，否则 showShellToast → 重渲 → #185
  const notice = useMemo(
    ...
    [networkOnline, habitatConnection, localPrefer],
  );
  const noticeKind = notice?.kind ?? null;

  // 设置页保存 Habitat 时 Chat/Diary 可能未挂载；条幅级监听才能强制重连
  useEffect(() => subscribeBundledHabitatRpcConfigChanges(), []);

  useEffect(() => {
    // sonner 走 useSyncExternalStore；在 commitPassiveMount 同步 toast 会 forceStoreRerender → #185
    const timer = window.setTimeout(() => {
      ...
      if (noticeKind === "local-prefer") {
        showShellToast(SHELL_TOAST_IDS.connectivity, "网络响应较慢，已改用本地数据与排队同步。", {
          ...
          action: {
            ...
            onClick: () => {
              clearLocalPrefer();
              reconnectHabitatOnce(reconnectingRef);
            },
          },
        });
        return;
      ...
      if (noticeKind === "habitat-connecting") {
  ...
  }, [canOpenHabitatSettings, noticeKind]);

...
import { getFtsStatus, getRebuildFtsJobStatus, startRebuildFtsIndex } from "@console/lib/api.ts";
...
function phaseLabel(phase: string): string {
  if (phase === "semantic_memory_segmented") return m.console_fts_semantic_seg();
  if (phase === "messages_segmented") return m.console_fts_messages_seg();
  if (phase === "semantic_memory_embedding") return m.console_fts_semantic_emb();
  if (phase === "messages_embedding") return m.console_fts_messages_emb();
  return phase;
}
  ...
  return (
    <div className="overflow-x-auto">
      <Table className="font-mono text-xs">
        <TableHeader>
          <TableRow>
            <TableHead>{m.console_common_table()}</TableHead>
            <TableHead>{m.console_common_capability()}</TableHead>
            <TableHead>FTS</TableHead>
            <TableHead>{m.console_common_segmented()}</TableHead>
            <TableHead>trgm</TableHead>
            <TableHead>{m.console_common_embedding()}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {rows.map((row) => (
            <TableRow key={row.table}>
              ...
              <TableCell className="text-muted-foreground whitespace-nowrap">
                {[
                  ...
                  row.capabilities.segmented && m.console_common_segmented(),
                  row.capabilities.trgm && "trgm",
                  row.capabilities.embedding && m.console_common_embedding(),
                ]
                  .filter(Boolean)
                  .join(" · ")}
              </TableCell>
      ...
      {!cjkEnabled ? (
        <p className="text-xs text-muted-foreground mt-2 font-sans">* {m.console_fts_cjk_note()}</p>
      ) : null}
    </div>
  );
}
...
function RebuildProgress({ job }: { job: FtsRebuildJobStatus }) {
  ...
  const label = job.phase ? phaseLabel(job.phase) : m.console_common_preparing();

  return (
    <Card className="bg-muted py-0 mb-4">
      <CardContent className="gap-3 py-4 px-4">
        <h3 className="font-bold text-sm">{m.console_fts_rebuild_progress()}</h3>
        {job.running ? (
          <>
            <p className="text-sm font-sans">
              {label} · {formatRatio(job.current, job.total)}
              {job.only_missing ? m.console_common_only_missing() : m.console_common_full_rebuild()}
            </p>
            <progress className="w-full h-2 accent-primary" value={pct} max={100} />
            <p className="text-xs text-muted-foreground font-sans">
              {m.console_common_background_hint()}
            </p>
          </>
        ) : null}
...
function FtsPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-4">{m.console_fts_title()}</h2>
      <p className="text-sm text-muted-foreground mb-4">{m.console_fts_desc()}</p>
      ...
      <Card className="bg-muted py-0 mb-4">
        <CardContent className="gap-3 py-4 px-4">
          <h3 className="font-bold text-sm">{m.console_fts_coverage()}</h3>
          {status?.coverage?.tables?.length ? (
            <CoverageTable rows={status.coverage.tables} cjkEnabled={status.enabled} />
          ) : (
            <p className="text-sm text-muted-foreground">{m.console_fts_pg_unavailable()}</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="bg-muted py-0 mb-4">
        <CardContent className="gap-3 py-4 px-4">
          <h3 className="font-bold text-sm">{m.console_common_config()}</h3>
          {status ? (
            ...
            <p className="text-sm text-muted-foreground">{m.console_common_load_failed_short()}</p>
          )}
          <div className="flex flex-wrap gap-2">
            ...
            >
              {job?.running ? m.console_fts_rebuilding() : m.console_fts_resume()}
            </Button>
            <Button
              type="button"
              variant="outline"

...
function stepLabel(stepId: string): string {
  switch (stepId) {
    case "light-sleep":
      return m.console_sleep_step_light_sleep();
    case "deep-sleep":
      return m.console_sleep_step_deep_sleep();
    case "dream":
      return m.console_sleep_step_dream();
    case "memory-ref-sync":
      return m.console_sleep_step_memory_ref_sync();
    case "self-layer-refresh":
      return m.console_sleep_step_self_layer_refresh();
    default:
      return stepId;
  }
}
...
function triggerLabel(trigger: string): string {
  switch (trigger) {
    case "scheduled":
      return m.console_sleep_trigger_scheduled();
    case "manual_cycle":
      return m.console_sleep_trigger_manual_cycle();
    case "manual_step":
      return m.console_sleep_trigger_manual_step();
    default:
      return trigger;
  }
}
...
function pipelineStatusLabel(status: string): string {
  switch (status) {
    case "completed":
      return m.console_common_success();
    case "failed":
      return m.console_common_failed();
    case "skipped":
      return status;
    default:
      return status;
...
function SleepPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-1">{m.console_nav_sleep()}</h2>
      <p className="text-sm text-muted-foreground mb-4">{m.console_sleep_desc()}</p>

      <Card className="bg-muted py-0 mb-4">
        <CardContent className="p-4">
          <h3 className="font-semibold mb-1">{m.console_sleep_cycle_title()}</h3>
          <p className="text-sm text-muted-foreground mb-3">{m.console_sleep_cycle_status()}</p>
          <FormFieldset bordered={false} className="gap-3 mb-3">
            <FormField label={m.console_sleep_cycle_day()} className="max-w-xs text-xs">
              <Input
                type="text"
                className="h-8"
            ...
            <div className="max-w-md">
              <FormFieldLabel className="text-xs py-0">
                {m.console_sleep_deep_sleep_mode()}
              </FormFieldLabel>
              <Select
                value={deepSleepMode}
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="full">{m.console_sleep_deep_sleep_mode_full()}</SelectItem>
                  <SelectItem value="incremental">
                    {m.console_sleep_deep_sleep_mode_incremental()}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="flex items-center gap-2 flex-wrap mb-4">
            ...
            >
              {pipelineStatus?.running || pipelineStarting
                ? m.console_sleep_cycle_running()
                : m.console_sleep_cycle_run()}
            </Button>
          ...
          {stepNodes.length > 0 && (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{m.console_sleep_cycle_step()}</TableHead>
                      ...
                      <TableRow key={node.id}>
                        ...
                        <TableCell className="text-right">
                          ...
                          >
                            {isRunningThis
                              ? m.console_sleep_cycle_running()
                              : m.console_sleep_cycle_step_run()}
                          </Button>
                        </TableCell>
                      </TableRow>
                    );
      ...
      <div className="flex items-center gap-2 mb-3">
        <h3 className="font-semibold flex-1">{m.console_sleep_pipeline_history_title()}</h3>
        <Button
          type="button"
          variant="ghost"
          size="sm"
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>{m.console_sleep_cycle_step()}</TableHead>
              <TableHead>{m.console_sleep_trigger()}</TableHead>
              <TableHead>{m.console_common_processing_day()}</TableHead>
              <TableHead>{m.console_common_status()}</TableHead>
              <TableHead>{m.console_sleep_attempt()}</TableHead>
              <TableHead>{m.console_common_tools()}</TableHead>
              <TableHead />
            </TableRow>
          </TableHeader>
          <TableBody>

import type { SelfLayerStorePort } from "@freeanima/engine-repos";

import { pgProfileWrap } from "../pg-profile.ts";
import * as crudRepo from "./repos/self-crud-repo.ts";

/** PostgreSQL SelfLayerStorePort 实现 */
export class PgSelfLayerStore implements SelfLayerStorePort {
  async getBlock(key: Parameters<SelfLayerStorePort["getBlock"]>[0]) {
    return pgProfileWrap("selfLayer.getBlock", () => crudRepo.getSelfBlock(key));
  }

  async listBlocks() {
    return pgProfileWrap("selfLayer.listBlocks", () => crudRepo.listSelfBlocks(), {
      resultBytes: (rows) => JSON.stringify(rows).length,
    });
  }

  async upsertBlock(input: Parameters<SelfLayerStorePort["upsertBlock"]>[0]) {
    return pgProfileWrap("selfLayer.upsertBlock", () => crudRepo.upsertSelfBlock(input));
  }

  async updateBlock(
    input: Parameters<SelfLayerStorePort["updateBlock"]>[0],
    opts?: Parameters<SelfLayerStorePort["updateBlock"]>[1],
  ) {
    return pgProfileWrap("selfLayer.updateBlock", () => crudRepo.updateSelfBlock(input, opts));
  }

  async isInitialized() {
    return pgProfileWrap("selfLayer.isInitialized", () => crudRepo.isSelfLayerInitialized());
  }
}

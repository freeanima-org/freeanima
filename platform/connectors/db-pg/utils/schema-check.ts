...
export async function assertMessagesSchema(db: Db): Promise<void> {
  ...
  if (pkCols.length !== 1 || pkCols[0] !== "id") {
    throw new Error(
      `messages table PK should be (id), currently (${pkCols.join(", ") || "unknown"}).` +
        "Run: bun run --filter @freeanima/core/db db:migrate",
    );
  }
  ...
  if (!posCol[0]?.exists) {
    throw new Error(
      "messages table missing pos column (in-session sequence). Run: bun run --filter @freeanima/core/db db:migrate",
    );
  }
  ...
  if (!payloadCol[0]?.exists) {
    throw new Error(
      "messages table missing payload column. Run: bun run --filter @freeanima/core/db db:migrate",
    );
  }
  ...
  if (Number(uqRows[0]?.cnt ?? 0) < 1) {
    throw new Error(
      "messages table missing unique index on (session_id, pos). Run: bun run --filter @freeanima/core/db db:migrate",
    );
  }
}

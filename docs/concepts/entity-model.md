...
Accounts store SMTP/IMAP settings and sync cursor in `body.sync`. Messages store IMAP UID in `body.imap_uid`; human-readable subject uses entity `title`. Sync pipeline: RFC822 → CTE/charset decode → strip attachments to disk → store **decoded content raw** in entity `content` (`text/plain` or `text/html`, see `body.content_type`); pure text always in `body.text` (preview / ToolSet default). Do **not** keep the full RFC822 blob on the entity. `email_read` / `email.message.read`: default body = plain text; `raw=true` = content raw (html or plain). UI sandboxes `text/html` via iframe（默认白底）. Also returns `headers`（SMTP/MIME map，小写键；不含已在实体字段中的 From/To/Subject/Date）and `attachments`（落盘于 `FREEANIMA_HOME/email-attachments/{account_id}/{message_id}/`，元信息在 `body.attachments`）. Shell sidebar mirrors common mail apps: Inbox (`direction=inbound`, unread/all) vs Sent (`direction=outbound`, no unread filter). `email_search` supports optional `from` / `sent_after` / `sent_before`（AND with query）. IMAP sync upserts threads/messages; UI lives in shell `/email` (SAP `email.*` methods), not Habitat REST.

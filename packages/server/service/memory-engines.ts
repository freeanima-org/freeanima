...
} from "@freeanima/capabilities/self/refresh-engine-port";
import {
  registerRetainLlm,
  type RetainLlmInput,
  type RetainLlmResult,
} from "@freeanima/capabilities/memory/service/retain-llm-port";
import {
  registerReflectLlm,
  type ReflectLlmInput,
  type ReflectLlmResult,
} from "@freeanima/capabilities/memory/service/reflect-llm-port";
...
async function resolveEngineSubjectId(opts: {
  ...
  const { getActiveRetainAgentSubjectId } =
    await import("@freeanima/capabilities/memory/service/retain-context.ts");

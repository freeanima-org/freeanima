...
export const Route = createFileRoute("/chamber/conversations/")({
  component: ChamberConversationsPage,
});

function ChamberConversationsPage() {
  ...
  useEffect(() => {
    void useChamberConversationsStore.getState().fetchConversations();
  }, []);

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-bold">{m.webui_chamber_nav_conversations()}</h2>
        <button
          type="button"
          ...
          <div className="card-body p-0">
            {totalConversations === 0 ? (
              <div className="text-sm text-base-content/50 p-4">
                {m.webui_chamber_conversations_empty()}
              </div>
            ) : (
              <>

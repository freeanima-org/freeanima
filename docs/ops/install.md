...
| 组件           | 版本 / 说明                                                                                                                                                |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Bun**        | >= 1.4.2 — **源码**安装必需（[bun.sh](https://bun.sh)；Windows：`winget install Oven-sh.Bun`）；独立二进制不需要                                           |
...
**前置：** Bun >= 1.4.2 · PostgreSQL（pgvector）· Redis（推荐）· Vault（推荐）· [just](https://github.com/casey/just)（`PATH` 上有 bash —— Windows 用 Git Bash）。Windows 搭建：[`windows-dev.md`](windows-dev.md)。

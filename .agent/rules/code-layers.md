...
| `src/frontend/shell-ui/`                      | `ui-kit`, `shell-sdk`, `feature-*`, `kernel`, `kernel-*`                                                                                                                  | **`rpc-contract`**；产品 UI 用 `@freeanima/features/*/ui/*`                |
...
- **Outpost companion**: overlay under `src/features/companion/ui`; product UIs live in `features/*/ui`. Do not import `platform`, `runtime`, `core`, or arbitrary `capabilities-*` from UI — use rpc-contract + shell-sdk + ui-kit. 功能原型见 [`frontend-features.md`](frontend-features.md)。

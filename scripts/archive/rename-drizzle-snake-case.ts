...
const ROOT = join(import.meta.dir, "..", "packages/core/db/pg");
...
function transform(content: string): string {
  ...
  for (const [from, to] of RENAMES) {
    ...
    out = out.replace(new RegExp(`\\.${f}\\b`, "g"), `.${to}`);
    out = out.replace(new RegExp(`([{,\\s])${f}:`, "g"), `$1${to}:`);
    out = out.replace(new RegExp(`(["'])${f}\\1:`, "g"), `$1${to}$1:`);
  }
  return out;
}
...
console.log(`Updated ${changed} db-pg files`);

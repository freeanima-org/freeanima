...
async function loadKvScope(scope: SettingsStorageScope): Promise<unknown> {
  ...
  if (scopeId === "hub") {
    const [hubUrl, remoteAuthToken] = await Promise.all([
      Preferences.get({ key: HUB_URL_KEY }),
      Preferences.get({ key: REMOTE_AUTH_TOKEN_KEY }),
    ]);
...
async function saveKvScope(scope: SettingsStorageScope, value: unknown): Promise<void> {
  ...
  if (scopeId === "hub") {
    ...
    await Preferences.set({ key: HUB_URL_KEY, value: raw.hubUrl });
    await Preferences.set({ key: REMOTE_AUTH_TOKEN_KEY, value: raw.remoteAuthToken });
    return;
  }
  if (scope.id === "debug") {
    ...
    await Preferences.set({ key: DEBUG_SENTRY_DSN_KEY, value: cfg.sentryDsn });
    await Preferences.set({

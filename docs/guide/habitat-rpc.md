...
**Habitat RPC** is the **single business channel** between Habitat clients and the agent runtime. The TypeScript constant is `HABITAT_RPC_VERSION`; the wire connect literal may still accept the historical value `"HubRPC/1.0"` for compatibility. Transport:
...
Server: [`src/platform/remote-tools/`](../../src/platform/remote-tools/). Client helper: `createRemoteToolsHub` in `@freeanima/shared/rpc-contract`.

## Endpoints

```text
ws://{hub_host}:{port}/rpc/v1
http://{hub_host}:{port}/rpc/v1/task/list
```

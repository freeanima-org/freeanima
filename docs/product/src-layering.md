...
2. oxlint 把整棵 `host/` 当一层；且不拦 `shared → host/core`
3. 部分 UI 值导入 `host/core/db/schema` barrel
4. `shared` 内混有 DOM 与 `node:fs` 入口
5. 存储 Zod 与 rpc-contract 平行手写易漂移
...
| P1   | #17723 | 剪 shared→host、frontend→db；oxlint 护栏  |
| P2   | #17724 | 纯 Zod 出口 + codegen/exports             |

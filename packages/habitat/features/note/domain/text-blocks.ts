...
import { groupNoteBlockHitsByParent, toNoteHitBlock } from "./search-hit.ts";
import type {
  NoteStoreContext,
  NoteTextBlock,
  NoteTextBlockCreateInput,
  NoteTextBlockReorderItem,
  NoteTextBlockUpdateInput,
} from "./types.ts";
...
async function assertNoteContainer(parentId: number, worldId: number): Promise<void> {
  const parent = await getEntity(parentId);
  if (!parent || parent.primary_component !== NOTE_COMPONENT) {
    throw new Error("parent must be note");
  }
...
export async function listNoteTextBlocks(
  ...
  return result.results
    .map((row) => mapHit(row))
    .filter((row): row is NoteTextBlock => row != null)
    .toSorted((a, b) => a.sort_order - b.sort_order || a.id - b.id);
}

export async function createNoteTextBlock(
  ctx: NoteStoreContext,
  input: NoteTextBlockCreateInput,
): Promise<NoteTextBlock> {
  ...
  await assertNoteContainer(input.parent_id, ctx.worldId);

  let sortOrder = input.sort_order;
  if (sortOrder == null) {
    const existing = await listNoteTextBlocks(ctx, input.parent_id);
  ...
  if (!mapped) throw new Error("note text block create failed");
  return mapped;
}

export async function updateNoteTextBlock(
  ctx: NoteStoreContext,
  input: NoteTextBlockUpdateInput,
): Promise<NoteTextBlock | null> {
  ...
  await assertNoteContainer(parsed.parent_id, ctx.worldId);
...
export async function deleteNoteTextBlock(ctx: NoteStoreContext, id: number): Promise<boolean> {
...
export async function deleteAllNoteTextBlocks(
  ctx: NoteStoreContext,
  parentId: number,
): Promise<void> {
  const blocks = await listNoteTextBlocks(ctx, parentId);
  for (const block of blocks) {
    await deleteEntity(block.id);
  }
}

export async function reorderNoteTextBlocks(
  ctx: NoteStoreContext,
  items: NoteTextBlockReorderItem[],
): Promise<NoteTextBlock[]> {
  if (items.length === 0) return [];
  const updated: NoteTextBlock[] = [];
  for (const item of items) {
    const row = await updateNoteTextBlock(ctx, {
      id: item.id,
      sort_order: item.sort_order,
    });
    if (!row) throw new Error(`note text block not found: ${item.id}`);
    updated.push(row);
  }
  return updated;
}

export type NoteTextBlockSearchOpts = {
  query: string;
  limit: number;
};

/**
 * 按 text 块正文 hybrid 搜索；按笔记 parent 分组保序。
 */
export async function searchNoteTextBlockHits(
  ctx: NoteStoreContext,
  opts: NoteTextBlockSearchOpts,
): Promise<{ parentId: number; blocks: NoteTextBlock[] }[]> {
  ...
  const hits: NoteTextBlock[] = [];
  for (const row of result.results) {
    const block = mapHit(row);
    if (!block) continue;
    hits.push(toNoteHitBlock(block));
  }
  return groupNoteBlockHitsByParent(hits, parentLimit);
}

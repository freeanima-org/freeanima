...
async function loadStreamBridge() {
  return import("@freeanima/capabilities/outpost/transport/stream-bridge");
}
  ...
  try {
    ...
    try {
      ...
      if (!answer) {
        ...
        const { getDb } = await import("@freeanima/core/db/pg/client.ts");
        const { messages } = await import("@freeanima/core/db/schema");

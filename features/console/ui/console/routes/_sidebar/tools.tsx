...
function returnKindLabel(kind: ToolsStatusToolItem["return_kind"]): string {
  return kind === "text" ? m.console_tools_kind_text() : m.console_tools_kind_json();
}

function returnKindHint(kind: ToolsStatusToolItem["return_kind"]): string {
  return kind === "text" ? m.console_tools_return_text() : m.console_tools_return_json();
}
...
async function copyText(text: string, label: string): Promise<void> {
  try {
    ...
    console.info(m.console_common_copied({ label }));
  } catch {
    console.warn(m.console_common_copy_failed({ label }));
  }
}
  ...
  return (
    <Card className="bg-muted py-0 mb-4">
      <CardContent className="py-3 px-4 gap-2">
        <h3 className="text-sm font-semibold">{m.console_tools_default_loaded()}</h3>
        <p className="text-xs text-muted-foreground">{m.console_tools_default_loaded_hint()}</p>
    ...
    <Card className="bg-muted py-0">
      <CardContent className="py-3 px-4">
          ...
          <div className="flex items-center gap-1 shrink-0 flex-wrap justify-end">
            ...
            {tool.requires_env?.length ? (
              <Badge variant="warning" className="text-xs">
                {m.console_tools_needs_secret()}
              </Badge>
            ) : null}
            {missingContract ? (
              <Badge variant="destructive" className="text-xs">
                {m.console_tools_no_contract()}
              </Badge>
            ) : null}
          </div>
        </div>
        ...
        {tool.return_schema ? (
          <details className="mt-1">
            <summary className="text-xs cursor-pointer text-muted-foreground">
              {m.console_tools_success_schema()}
            </summary>
            <pre className="text-xs mt-1 bg-muted p-2 rounded overflow-x-auto">
        ...
        {exampleText ? (
          <details className="mt-1">
            <summary className="text-xs cursor-pointer text-muted-foreground flex items-center gap-2">
              <span>{m.console_tools_fidelity_example()}</span>
              <Button
                type="button"
                variant="ghost"
                size="sm"
                ...
                onClick={(e) => {
                  e.preventDefault();
                  void copyText(exampleText, m.console_tools_success_example());
                }}
              >
        ...
        <details className="mt-1">
          <summary className="text-xs cursor-pointer text-muted-foreground">
            {m.console_tools_error_return()}
          </summary>
          <p className="text-xs text-muted-foreground mt-1">{m.console_tools_error_unified()}</p>
          <pre className="text-xs mt-1 bg-muted p-2 rounded overflow-x-auto">
            {JSON.stringify(tool.error_schema, null, 2)}
          </pre>
          <p className="text-xs text-muted-foreground mt-2">{m.console_tools_example()}</p>
          <pre className="text-xs mt-1 bg-muted p-2 rounded overflow-x-auto">
...
function ToolsPage() {
  ...
  if (toolSets.length === 0) {
    return (
      <div>
        <h2 className="text-lg font-bold mb-4">{m.console_nav_tools()}</h2>
        <p className="text-sm text-muted-foreground mb-4">{m.console_tools_desc()}</p>
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-4">{m.console_nav_tools()}</h2>
      <p className="text-sm text-muted-foreground mb-4">{m.console_tools_desc_grouped()}</p>
      <DefaultToolSetsSection names={defaultToolSets} />
      ...
      <div className="space-y-4">
        ...
        {ungroupedTools.length > 0 ? (
          <>
            <h3 className="text-sm font-bold mt-4 mb-2">{m.console_tools_ungrouped()}</h3>
            <div className="space-y-3">

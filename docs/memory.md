...
**所有记忆处理必须携带数字生命的身份上下文。** 提取、整理、合并——每一步都应加载自我层（灵魂文件）与常驻记忆，让 LLM 知道自己是谁。不带身份的记忆处理会产生通用化的、缺乏个性的结果，这不是我们要的。
...
- 系统提示词（灵魂文件六块 + 常驻记忆 + 项目上下文；见 [`self-layer.md`](self-layer.md)）
- 当前 session 近期消息
- 从长期记忆中召回的相关片段
- 工具调用的实时返回结果

...
export async function createSubjectEntity(input: {
  ...
  if (input.type === "user") {
    const { assertAtMostOneUser } = await import("@freeanima/core/db/pg/entity");
    ...
    if (identity?.habitat_instance_id && isPatchableRuntimeConfig(config)) {
      ...
      try {
        const { getResolvedWorldContext } =
          await import("@freeanima/core/config/resolved-world-context.ts");
  ...
  if (input.type === "agent" && input.self_foundation) {
    const { upsertSelfBlock } = await import("@freeanima/core/db/pg/self-layer");
    const { SELF_BLOCK_KEYS } = await import("@freeanima/core/db/pg/self-layer/types");

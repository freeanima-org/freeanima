...
function maskSecret(key: string, value: unknown): string {
  ...
  if (
    ...
    return s ? `${s.slice(0, 8)}…` : m.console_common_empty();
  }
  return s || m.console_common_empty();
}
    ...
    return (
      <Card className="bg-muted py-0">
        <CardContent className="gap-3 py-4 px-4">
          ...
          {rows.length === 0 ? (
            <p className="text-xs text-muted-foreground">{m.console_common_empty()}</p>
          ) : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-48">{m.console_common_key_label()}</TableHead>
                    <TableHead>{m.console_common_value_label()}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
...
function ConfigPage() {
  ...
  if (config == null) {
    return (
      <div>
        <h2 className="text-lg font-bold mb-4">{m.console_nav_config()}</h2>
        <StatusAlert variant="error">{m.console_common_load_failed_short()}</StatusAlert>
      </div>
    );
  }
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-4">{m.console_nav_config()}</h2>
      <p className="text-sm text-muted-foreground mb-4">{m.console_config_desc()}</p>
      {blocks.length === 0 ? (
        <StatusAlert variant="info">{m.console_config_empty()}</StatusAlert>
      ) : (

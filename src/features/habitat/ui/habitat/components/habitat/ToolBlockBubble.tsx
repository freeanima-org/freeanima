...
function ToolCallRow({ call }: { call: DisplayToolCall }) {
  ...
  return (
    <Collapsible isExpanded={open} onExpandedChange={setOpen}>
      ...
      <CollapsibleContent>
        <div className="rounded-lg bg-background/60 p-2 space-y-1.5 min-w-0 mt-1 mb-2">
  ...
  return (
    <div className="tool-bubble text-xs">
      <Collapsible isExpanded={expanded} onExpandedChange={setExpanded}>
        <CollapsibleTrigger className="w-full text-left px-3 py-2 flex items-center gap-2 hover:bg-muted/40 rounded-2xl transition-colors min-w-0">
          ...
          <div className="flex-1 min-w-0">
            {expanded ? (
              ...
              <div className="overflow-hidden whitespace-nowrap max-w-full font-medium text-muted-foreground">
                <span className={isActive ? "inline-block animate-pulse" : "truncate block"}>
                  {headline}
                </span>
              </div>
            )}
        ...
        <CollapsibleContent>
          <div className="border-t border/50 px-3 py-2 space-y-1">
            {calls.map((c, ci) => (

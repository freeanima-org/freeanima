...
见 [`docs/modules/object-storage.md`](../modules/object-storage.md)（持久本地库 /
远端 SSOT、无预签名）。
...
HTTP（Bearer）：仅 `object_storage.file.get`（按 entity id；ETag=cid 协商
304）。空配置时用本机持久库；填齐 S3 则远端为 SSOT。

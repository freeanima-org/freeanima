...
export function buildEmailMailboxToolDefs(io: EmailToolIo) {
          ...
          try {
            ...
            const messages = await listEmailMessages(
              ...
              omitUndefined({
                ...
                imap_mailbox:
                  args.mailbox != null ? coerceString(args.mailbox).trim() || undefined : undefined,
          ...
          try {
            ...
            const messages = await searchEmailMessages(
              ...
              omitUndefined({
                ...
                subject:
                  args.subject != null ? coerceString(args.subject).trim() || undefined : undefined,
                since:
              ...
              (await resolveEmailToolTagIds(
                ...
                omitUndefined({
                  ...
                  tag_ids: Array.isArray(args.tag_ids)
                    ? args.tag_ids.map((x) => Number(x)).filter((n) => Number.isFinite(n) && n > 0)
                    : undefined,
                }),
              )) ?? [];
            if (target === "thread") {

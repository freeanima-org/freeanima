...
import { PROMPT_XML_TAGS, type SystemPromptSection } from "@freeanima/core/hooks/prompt";
...
export function registerRoomProtocolSystemPromptHook(ctx: Context): void {
  onSystemPromptBuild(ctx, async (event) => {
    if (event.meta?.scenario !== "room_inner") return undefined;
    ...
    const roomId = event.meta.room_id?.trim();
    if (roomId) {
      try {
        ...
        if (room) {
          const body = await buildRoomMembersPromptBody({
            ...
            self_public_id: event.meta.agent_public_id ?? null,
          });
          if (body.trim()) {
            sections.unshift({
              id: "room-members",

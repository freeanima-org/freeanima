...
- IDLE on inbox + cron `builtin-email-sync-all` every 5m across **all worlds** (enabled accounts); auto-sync new inbox subjects → one user notification; manual `email.sync` does not notify

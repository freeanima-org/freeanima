...
import {
  buildAcpCallbackPrompt,
  createAcpSessionUpdatedHandler,
} from "./acp-conversation-callback.ts";

describe("buildAcpCallbackPrompt", () => {
  it("includes acp_conversation_id per task", () => {
    const prompt = buildAcpCallbackPrompt([
      {
        acp_conversation_id: "acp-1",
        status: "completed",
        task_id: "t1",
        agent_name: "cursor",
  ...
  it("schedules recheck when notify arrives during inflight", async () => {
    ...
    const handler = createAcpSessionUpdatedHandler({
      conversation: {
        ...
        updateConversationMetaField: async (_sid: string, patch: Record<string, unknown>) => {

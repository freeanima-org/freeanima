  ...
  return (
    <div className={isReader ? "h-full flex flex-col min-h-0 overflow-hidden" : undefined}>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4 shrink-0">
        <div>
          <h2 className="text-lg font-bold">{m.admin_nav_email()}</h2>
          <p className="text-sm text-base-content/60 mt-1">
            {view === "picker" ? m.admin_email_desc_picker() : m.admin_email_desc_reader()}
          </p>
        </div>
        <div className="flex gap-2">
          {view === "reader" ? (
            <button type="button" className="btn btn-sm btn-ghost" onClick={backToPicker}>
              {m.admin_email_back_accounts()}
            </button>
          ) : null}
          <button
            type="button"
      ...
      {errors._all ? (
        <div className="alert alert-warning text-sm mb-4 shrink-0">
          {m.admin_email_partial_load_failed({ detail: errors._all })}
        </div>
      ) : null}
      ...
      {loading ? (
        ...
        <div className="alert alert-info text-sm">{m.admin_email_no_accounts()}</div>
      ) : view === "picker" ? (
            ...
            <div key={account.id} className="card bg-base-200">
              <div className="card-body">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  {account.default_sender ? (
                    <span className="badge badge-sm badge-primary">
                      {m.admin_email_default_sender()}
                    </span>
                  ) : null}
                  {account.enabled === false ? (
                    <span className="badge badge-sm badge-ghost">{m.admin_common_disabled()}</span>
                  ) : (
                    <span className="badge badge-sm badge-success">{m.admin_email_enabled()}</span>
                  )}
                </div>
                ...
                <table className="table table-xs mb-3">
                  <tbody>
                    <tr>
                      <td className="text-base-content/50 w-20">{m.admin_email_address()}</td>
          ...
          <div className="flex flex-col min-h-0 overflow-hidden bg-base-200/50 max-lg:flex-1 lg:w-80 lg:min-w-[280px] lg:shrink-0 lg:border-r border-base-300 max-lg:border-b">
            ...
            <div className="flex-1 overflow-y-auto">
              {listLoading ? (
                ...
                <p className="text-xs text-base-content/50 p-3">{m.admin_email_no_messages()}</p>
              ) : (
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-sm truncate">
                            {msg.subject || m.admin_email_no_subject()}
                          </div>
              ...
              <>
                <div className="shrink-0 p-4 border-b border-base-300">
                  <h3 className="font-bold text-base">
                    {detail.subject || m.admin_email_no_subject()}
                  </h3>
                  <div className="text-sm text-base-content/70 mt-2 space-y-1">
                    <div>
                      <span className="text-base-content/50">{m.admin_email_from()}</span>
                      {detail.from}
                    </div>
                    <div>
                      <span className="text-base-content/50">{m.admin_email_to()}</span>
                      {detail.to}
                    </div>
                    <div>
                      <span className="text-base-content/50">{m.admin_email_date()}</span>
                      {formatDisplayDateTime(detail.date)}
                    </div>
                  </div>
                </div>
                <pre className="flex-1 min-h-0 overflow-y-auto p-4 text-sm whitespace-pre-wrap font-sans">
                  {detail.body || detail.preview || m.admin_email_no_body()}
                </pre>
              </>
            ) : (
              <div className="flex justify-center items-center flex-1 min-h-0 text-sm text-base-content/50">
                {m.admin_email_select_message()}
              </div>
            )}
          </div>
        </div>

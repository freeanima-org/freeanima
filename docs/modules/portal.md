...
- 代码：`src/portal/extension`（WXT MV3；React popup/options + `runtime/` + `features/*`；图标 `public/icon-*.png`）
...
- gecko id：`extension@freeanima.com`（对齐 `src/portal/extension`；**变更 id = 新扩展**，已装旧 id 须卸旧装新并重配连接）

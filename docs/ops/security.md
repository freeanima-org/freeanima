...
| 聊天室 User vault 解锁 | **仅 v1 内嵌聊天室**（`packages/portal/app/web` / desktop / mobile）；Discord / 微信 Gateway **不能**解锁 User 库                                                                                                                                                                                                                  |
| 浏览器扩展             | 浏览器形态入口（`packages/portal/extension`；展示名 FreeAnima；Firefox gecko `extension@freeanima.com`）；Service API Token 存 `chrome.storage.local`；主密码仅扩展内存（最多 8h 绝对超时；浏览器关闭清 session）；RPC 走 background `host_permissions`。见 [`portal.md`](../modules/portal.md)、[`vault.md`](../modules/vault.md) |

...
| ------------------------------------------------------ | ------------------------- | ---------------------------------- |
| 任务 **到期（`due_at`）**                              | 是（该 World 的 subject） | 也可经 `notification.created` 打断 |
| 任务 **提前提醒**（`reminders[]`，锚点 start/end/due） | **否**                    | **仅是**                           |
| 日历事件 **reminders / 开始**                          | 是（该 World 的 subject） | 与任务同一扫描；提醒相对开始       |
...
| 循环任务     | 存活 `task_item` 保持 `pending`，完成时滚动计划时钟 / reminders（见 [`docs/modules/task.md`](../modules/task.md)）；扫描器仍只看 pending live，滚期后可再次触发 |

其他内置（记忆维护、env-health、用户 Agent cron）可继续用现有 cron 表机制。本切面只把**任务到期 / 提醒发现**移出该路径。
...
2. memory-maintenance / env-health / email-sync-all / temporal-summary-tick 仍用 **进程内 `Bun.cron`**（非 PG `cron_jobs`）。失败时 Inbox 双收件策略不变。

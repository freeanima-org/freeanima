...
Derive WS URL from HTTP: replace `http` with `ws`, append `/rpc/v1`. Helpers: `resolveHabitatRpcWsUrl`, `buildHabitatRestRequest` in `@freeanima/shared/habitat-rpc`.

Habitat route: [`src/platform/sap/bun-route.ts`](../../src/platform/sap/bun-route.ts).

## HTTP REST mapping

Habitat method `domain.action` maps to path **`/rpc/v1/domain/action`** (dots → slashes; camelCase preserved).

| Kind           | HTTP     | Example                                                         |
| -------------- | -------- | --------------------------------------------------------------- |
| Read-only      | **GET**  | `GET /rpc/v1/task/list?subject_kind=user`                       |
| Write          | **POST** | `POST /rpc/v1/task/create` `{ subject_kind, title, … }`         |
| By id (read)   | **GET**  | `GET /rpc/v1/task/get/42?subject_kind=user`                     |
| By id (write)  | **POST** | `POST /rpc/v1/task/patch/42` `{ subject_kind, title?, … }`      |
| Sensitive read | **POST** | `POST /rpc/v1/vault/get/3` `{ subject_kind, include_secrets? }` |
...
Method contracts live in [`src/shared/habitat-contract/registry/`](../../src/shared/habitat-contract/registry/) (`transports: http | ws`; HTTP routes in `meta.http`). Handlers register via `feature/plugin.hub.rpc`.

## Authentication

Every **WebSocket** connection must send a valid **service API token** in the `connect` payload (`auth_token`). **HTTP REST** uses `Authorization: Bearer fa_at_...` on each request unless the method registry marks `auth: optional` (public read-only probes only). The Habitat verifies with `verifyServiceApiToken` ([`src/core/db/pg/service-api-token/`](../../src/core/db/pg/service-api-token/)).

Bundled clients read the token from `window.satelliteShell.remoteAuth.token` (shell bridge), configured in client **Habitat settings** (`/settings`). Habitat `/web/config.json` does not include tokens.
...
| Encoding             | Use                                                                                                             |
| -------------------- | --------------------------------------------------------------------------------------------------------------- |
| `response: raw`      | Handler returns `Response` (PEM, PNG, VRM, MPEG, octet-stream). Client: `callRaw()` or `fetchHabitatRestRaw()`. |
...
Helpers: `habitatRestUrl()`, `binaryHttpMeta()` / `rawPublicHttpMeta()` in habitat-contract. JSON methods continue to use `hubClient.call()`.
...
Use `runHubRpcTransport` for connect, heartbeat, and exponential backoff (default 1s → 30s cap). On satellite detach or socket close, Habitat unregisters tools for that instance.

...
本目录是 **UI/UX 规范 SSOT**。Agent 硬禁令、导入规则与 API 入口路径在 [`.cursor/rules/`](../../.cursor/rules/agent-bootstrap.mdc) 并链到此处。Token 值在代码中（`packages/ui-kit/styles/globals.css`）。代码与文档冲突时，**以代码为准**。
...
| `packages/ui-kit/`              | 组件 + CSS token（实现 SSOT）              | 产品架构             |

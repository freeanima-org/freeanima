...
import { getDb } from "@freeanima/core/db/pg/client.ts";
import {
  appendRoomMessage,
  deleteRoom,
  getRoom,
  ...
  updateRoomMembers,
  type RoomRow,
} from "@freeanima/core/db/pg/room";
import { conversations } from "@freeanima/core/db/schema";
import type { RoomMembersJson } from "@freeanima/core/db/schema";
import { listEntities } from "@freeanima/core/db/pg/entity";
import { agentConfigBodySchema } from "@freeanima/core/db/schema/entity";
import { getResolvedWorldContext } from "@freeanima/core/config/resolved-world-context.ts";
import { appendMessage } from "@freeanima/engine/conversation/conversation-crud.ts";
import { PROMPT_XML_TAGS, wrapPromptXml } from "@freeanima/core/hooks/prompt";
import type {
  RoomMemberPayload,
  RoomMessagePayload,
  RoomSummaryPayload,
  ...
  if (input.federated) {
    ...
    if (role === "satellite") {
      ...
      const { requestFederationRpc } =
        await import("@freeanima/capabilities/federation/satellite-rpc.ts");
...
export async function prepareAgentTurn(
  ...
  try {
    ...
    const { getLastMessageRole } =
      await import("@freeanima/engine/conversation/conversation-crud.ts");

...
export function registerFeatures(entries: FeaturePlugin[]): void {
  for (const plugin of entries) {
    plugins.push(plugin);
    if (plugin.hub.rpc) {
      for (const [method, handler] of Object.entries(plugin.hub.rpc) as [
        SapMethod,
        FeatureRpcHandler,
      ][]) {
...
export function getFeatureRpcHandler(method: SapMethod): FeatureRpcHandler | undefined {

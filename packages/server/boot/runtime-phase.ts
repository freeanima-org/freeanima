import { initCronModule } from "@freeanima/capabilities/connectors/cron";
...
import { bindRemoteToolsServerDeps } from "@freeanima/capabilities/outpost/transport/runtime-context.ts";
import {
  bindFederationHubWsDeps,
  createFederationManager,
  resolveFederationRole,
} from "@freeanima/capabilities/federation";
import { HabitatSessionRegistry } from "@freeanima/capabilities/outpost/transport/habitat-session-registry.ts";
...
export async function bootRuntimePhase(
  ...
  const { registerSemanticClusteringEmbeddingHook } =
    await import("@freeanima/capabilities/memory/clustering/register-hook.ts");
  ...
  const { seedBuiltinSkills } = await import("@freeanima/core/skill");

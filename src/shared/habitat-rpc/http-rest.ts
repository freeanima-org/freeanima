...
export type BuildHabitatRestRequestOptions = {
  body?: BodyInit;
  /** GET 条件请求：对应 If-None-Match */
  ifNoneMatch?: string;
};

export function habitatRestUrl(
  httpOrigin: string,
  method: string,
  payload: Record<string, unknown>,
): string {
  return buildHabitatRestRequest(httpOrigin, method, payload).url;
}

export function buildHabitatRestRequest(
  httpOrigin: string,
  method: string,
  payload: Record<string, unknown>,
  authToken?: string,
  ...
  const url = new URL(`${normalizeOrigin(httpOrigin)}${HABITAT_RPC_REST_PREFIX}/${restPath}`);
...
export type HabitatRestErrorBody = {
...
export async function parseHabitatRestResponse(res: Response): Promise<unknown> {
  if (res.status === 304) {
...
export function habitatTlsCaInfoUrl(httpOrigin: string): string {
  return habitatRestUrl(httpOrigin, "tls.ca.info", {});
}

/** TLS CA PEM 下载 URL（无 token） */
export function habitatTlsCaDownloadUrl(httpOrigin: string): string {
  return habitatRestUrl(httpOrigin, "tls.ca", {});
}

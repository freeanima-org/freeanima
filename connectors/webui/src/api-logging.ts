...
/** 组合根在 WebUI 启动前绑定进程 Logger（与 createServiceLogger 同一实例） */
export function bindWebuiApiLogging(logger: Logger): void {
  apiLogger = logger.with({ component: "api" });
  sseLogger = logger.with({ component: "sse" });
}

export function resetWebuiApiLoggingForTest(): void {
  apiLogger = null;
  sseLogger = null;
}

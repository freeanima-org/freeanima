...
import { resolvePrivateWorldId } from "@freeanima/core/config/world-context-pg.ts";

export function registerWorldContextSystemPromptHook(ctx: Context): void {
  onSystemPromptBuild(ctx, async (event) => {
    ...
    const currentAgentId = event.meta?.agent_subject_id;

  ...
  return (
    <details className="group border-t border-base-300 bg-base-100">
      ...
      <div className="px-4 pb-3">
        {!redisConfigured ? (
          <p className="text-xs text-warning">{m.admin_fridge_redis_unavailable()}</p>
        ) : !hasMagnets ? (
          <p className="text-xs text-base-content/50">{m.admin_fridge_no_notes()}</p>
        ) : (

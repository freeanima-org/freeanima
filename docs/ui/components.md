...
入口 UI 的分层与落点规则。实现位于 `packages/ui-kit/` 与 `packages/ui-features/*/ui/`。Agent 导入禁令 →
...
| 业务 / 产品 UI       | **领域（Domain）** UI                                               | `packages/ui-features/<slug>/ui` |
| UI 组件              | 上述某一层的 React 构建块                                           | —                                |
| UI 元素 / 槽位碎片   | 模式内部的展示部件（标题文案、徽章、图标）— 不是独立包              | 组件或模式内部                   |
| 应用 chrome          | **应用布局（app frame）**（不是壳）                                 | `packages/app-frame`             |
| 宿主                 | **壳（Shell）**                                                     | `packages/portal/app/*`          |

...
import { getSatellitesStatus } from "@freeanima/features/habitat/ui/habitat/lib/api.ts";
...
function SatellitesPage() {
  ...
  if (!status) {
    return (
      <div>
        <h2 className="text-lg font-bold">{m.habitat_nav_satellites()}</h2>
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          <h2 className="text-lg font-bold">{m.habitat_nav_satellites()}</h2>
          <p className="text-sm text-muted-foreground mt-1">{m.habitat_satellites_desc()}</p>
        </div>
        <Button
          type="button"
          variant="ghost"
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {(
          [
            [m.habitat_satellites_instances(), status.instance_count],
            [m.habitat_satellites_registered_tools(), status.tool_count],
          ] as const
        ).map(([label, count]) => (
      ...
      {status.instances.length === 0 ? (
        <StatusAlert variant="info">{m.habitat_satellites_empty_hint()}</StatusAlert>
      ) : (
            ...
            <Card key={`${inst.app_slug}:${inst.instance_id_norm}`} className="bg-muted py-0">
              <CardContent className="gap-3 py-4 px-4">
                ...
                <dl className="grid gap-2 text-sm sm:grid-cols-2">
                  <div>
                    <dt className="text-muted-foreground">{m.habitat_satellites_app_id()}</dt>
                    <dd className="font-mono text-xs break-all">{inst.app_id}</dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">{m.habitat_satellites_instance_id()}</dt>
                    <dd className="font-mono text-xs break-all">{inst.instance_id}</dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">{m.habitat_satellites_connected_at()}</dt>
                    <dd>{formatDisplayDateTime(inst.connected_at)}</dd>
                  </div>
                  <div>
                    <dt className="text-muted-foreground">
                      {m.habitat_satellites_last_heartbeat()}
                    </dt>
                ...
                <p className="text-sm font-medium">
                  {m.habitat_satellites_tools_count({ count: String(inst.tool_count) })}
                </p>
                {inst.tools.length > 0 ? (

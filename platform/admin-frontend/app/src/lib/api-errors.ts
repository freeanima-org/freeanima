...
export function translateApiPayload(payload: ApiWirePayload | null | undefined): string {
  if (!payload) return m.admin_common_request_failed();
  const code = payload.code;
  const params = payload.params ?? {};

  if (code === "cron_job_not_found") {
    return m.admin_api_cron_job_not_found({ id: params.job_id ?? "" });
  }
  if (code === "service_restarting") {
    return m.admin_api_service_restarting();
  }
  if (code === "email_account_not_found") {
    return m.admin_api_email_account_not_found({ account_id: params.account_id ?? "" });
  }
  if (code === "semantic_memory_count") {
    return m.admin_api_semantic_memory_count({ count: params.count ?? "" });
  }
  if (code === "studio_workspace_missing") {
    return m.admin_api_studio_workspace_missing();
  }
  if (code === "terminal_session_not_found") {
    return m.admin_api_terminal_session_not_found();
  }

  return payload.error ?? payload.message ?? m.admin_common_request_failed();
}

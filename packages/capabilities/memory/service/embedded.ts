...
import { getMessageTextItemsByIds } from "@freeanima/core/db/pg/conversation";
import {
  PROMPT_XML_TAGS,
...
export function createEmbeddedMemoryService(
  ...
  const service: MemoryService = {
    ...
    temporal: {
      async list(input = {}) {
        const { listTemporalSummaries } = await import("@freeanima/core/db/pg/temporal-summary");
      ...
      async get(input) {
        if (input.id != null) {
          const { getEntity } = await import("@freeanima/core/db/pg/entity");
        ...
        if (input.bucket && input.key) {
          const { getTemporalSummary } = await import("@freeanima/core/db/pg/temporal-summary");
          const { resolveToolCallerAgentWorldId } = await import("../tool-agent-world.ts");
          const { agent_world_id } = await resolveToolCallerAgentWorldId();

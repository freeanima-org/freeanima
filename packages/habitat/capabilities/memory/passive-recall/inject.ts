import type { AssistantMessage, StoredMessage } from "@freeanima/habitat/core/db/domain";
...
export const PASSIVE_MEMORY_CONTEXT_HEAD =
  "以下是与当前用户消息相关的语义记忆。有依据时使用，并在回复末尾引用 [[anima:id]]（id 取自 <memory id>）。";
...
export function formatPassiveMemoryBlock(hits: SemanticRecallHit[], maxChars: number): string {
  const { text } = renderSemanticMemoryList(
    hits.map((hit) => toSemanticMemoryPromptItem(hit)),
    { fields: CONVERSATION_MEMORY_FIELDS, maxChars },
  );
  return wrapPromptXml(PROMPT_XML_TAGS.passiveMemory, text);
}

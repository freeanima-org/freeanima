...
入口 UI 的视觉语言。Token **取值**仅定义在 [`packages/ui-kit/styles/globals.css`](../../packages/ui-kit/styles/globals.css)。本页定义**意图、用法与维度适配**。技术栈：Tailwind CSS v4 + shadcn/ui（`@freeanima/ui-kit`）。**不**使用 DaisyUI。

...
import { cstDayRange } from "@freeanima/capabilities/memory";
import { createEmbeddedMemoryService } from "@freeanima/capabilities/memory/service";
import { runRetainCatchUp } from "@freeanima/capabilities/memory/service/retain-catch-up";
import { planSleepCatchUp } from "@freeanima/capabilities/memory/sleep-catch-up";
import {
  runSelfLayerRefresh,
  runSelfLayerRefreshAllAgents,
} from "@freeanima/capabilities/self/refresh/run";
import type { BoundConversationAgent } from "@freeanima/engine/conversation/resolve-conversation-agent.ts";
...
import { gcObjectBlobsAfterEntityPurge } from "@freeanima/capabilities/object-storage/domain";
...
import { cacheGetJson, cacheSetJson } from "@freeanima/core/redis";
...
async function resolveMaintenanceAgents(
  ...
  const { assertBindableAgentSubject, listEnabledBoundAgents } =
    await import("@freeanima/engine/conversation/resolve-conversation-agent.ts");
...
async function runSemanticClusterCalibrateStep(ctx: StepCtx): Promise<MaintenanceStepResult> {
  const { calibrateSemanticMemoryClusters } =
    await import("@freeanima/capabilities/memory/clustering/calibrate.ts");
  try {
    const agents = await resolveMaintenanceAgents(ctx.agent_subject_id);
    if (agents.length === 0) {
      return {
...
async function runSemanticClusterTitleStep(ctx: StepCtx): Promise<MaintenanceStepResult> {
  try {
    const { listSemanticMemoryClusterStats } =
      await import("@freeanima/core/db/pg/search/clustering-repo.ts");
    const { warmSemanticClusterTitles } =
      await import("@freeanima/capabilities/memory/clustering/cluster-title.ts");
    const agents = await resolveMaintenanceAgents(ctx.agent_subject_id);
    if (agents.length === 0) {
      return {
        ok: true,

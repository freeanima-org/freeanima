...
import { getAcpStatus, startAllAcp, startAcp, stopAllAcp, stopAcp } from "@console/lib/api.ts";
import { m } from "@console/lib/i18n.ts";
import { acpStatusLabel } from "@console/lib/console-status.ts";
...
function AcpPage() {
  ...
  const controlAgent = async (name: string, action: "start" | "stop") => {
    ...
    try {
      ...
      const result =
        action === "start"
          ? await withTimeout(req, ACP_START_TIMEOUT_MS, m.console_acp_start_timeout())
          : await req;
      setStatus(result as AcpStatus);
    } catch (e) {
      logCaughtError("routes/_sidebar/acp", e);
      setError(
        m.console_acp_action_failed({
          name,
          action: action === "start" ? m.console_acp_connect() : m.console_acp_disconnect(),
  ...
  const controlAll = async (action: "start-all" | "stop-all") => {
    ...
    try {
      ...
      const result =
        action === "start-all"
          ? await withTimeout(req, ACP_START_TIMEOUT_MS, m.console_acp_start_all_timeout())
          : await req;
      setStatus(result as AcpStatus);
    } catch (e) {
      logCaughtError("routes/_sidebar/acp", e);
      setError(
        action === "start-all"
          ? m.console_acp_start_all_failed({
              detail: e instanceof Error ? e.message : String(e),
            })
          : m.console_acp_stop_all_failed({
              detail: e instanceof Error ? e.message : String(e),
            }),
      );
    } finally {
  ...
  if (!status) {
    return (
      <div>
        <h2 className="text-lg font-bold">{m.console_nav_acp()}</h2>
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          <h2 className="text-lg font-bold">{m.console_nav_acp()}</h2>
          <p className="text-sm text-muted-foreground mt-1">{m.console_acp_desc()}</p>
        </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            [m.console_acp_configured(), status.agent_count],
            [m.console_common_connected(), status.connected_count],
            [m.console_acp_active_sessions(), status.session_count],
            [m.console_acp_registered_tools(), status.tool_count],
          ].map(([label, value]) => (
      ...
      <StatusAlert variant="warning" className="text-xs mt-6">
        <p className="font-medium">{m.console_acp_notes_title()}</p>
        <ul className="list-disc list-inside mt-1 space-y-0.5 text-muted-foreground">
          <li>{m.console_acp_note_auto_connect()}</li>
          <li>{m.console_acp_note_handshake()}</li>
        </ul>
      </StatusAlert>

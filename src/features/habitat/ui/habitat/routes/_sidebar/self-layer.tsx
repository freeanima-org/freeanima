...
function SelfLayerPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-bold mb-1">{m.habitat_nav_self_layer()}</h2>
      <p className="text-sm text-muted-foreground mb-4">
        {m.habitat_self_layer_desc()} <code className="text-xs">update_self_block</code>
      </p>

      {blocks.length === 0 ? (
        <StatusAlert variant="info">{m.habitat_self_layer_empty()}</StatusAlert>
      ) : (
            ...
            return (
              <Card key={block.block_key} className="bg-muted py-0">
                <CardContent className="gap-3 py-4 px-4">
                  ...
                  >
                    {body || m.habitat_self_layer_not_set()}
                  </p>
                </CardContent>
              </Card>
            );

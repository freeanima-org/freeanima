...
import { ProviderError, type LlmCallParams } from "@freeanima/core/provider";
import type { HookClarifyItem, HookStreamEvent, TurnControl } from "@freeanima/core/hooks/loop";
import { runBeforeLlmCall, runToolAfterCall } from "@freeanima/core/hooks/cordis";
...
export type AfterMessagesPersisted = (ctx: {
...
export type EngineOpts = {
  ...
  /**
   * When true, install tool_progress ALS sink for this run's tool handlers.
   * Callers decide (e.g. conversation yes, auto_llm no); not derived from llm_kind.
   */
...
function prepareEngine(
  opts: Pick<EngineOpts, "model" | "tools" | "toolRegistry">,
  ...
  if (!opts.model) {
    throw new Error("EngineOpts.model is required");
  }
  return [schemas, opts.model];
}
...
async function afterMessagesPersisted(
  messages: StoredMessage[],
  batch: StoredMessage[],
  model: string,
  tools: OpenAiToolSchema[],
  opts?: Pick<EngineOpts, "onAfterMessagesPersisted">,
): Promise<void> {
  await opts?.onAfterMessagesPersisted?.({ messages, batch, model, tools });
}
 ...
 * `reportToolProgress` (e.g. subagent live steps). Enabled via opts.toolProgress.
 */
...
export async function* runStream(
  ...
  for (let iteration = 0; iteration < maxLoopIterations; iteration++) {
    ...
    if (!toolCalls?.length) {
      ...
      await persistMessages([pushed], opts);
      await afterMessagesPersisted(messages, [pushed], model, toolSchemas, opts);
          ...
          if (blockedByLoaded) {
            ...
            if (!validated.ok) {
              ...
              try {
                ...
                // Callers opt in (conversation); auto_llm inherits parent ALS reporter
                if (opts.toolProgress) {

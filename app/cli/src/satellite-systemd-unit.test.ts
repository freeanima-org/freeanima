...
describe("satellite systemd unit", () => {
  const launch: SatelliteLaunch = {
    ...
    environment: {
      ...
      STUDIO_WORKSPACE: "/home/user/project",
    },
    execStart: "bun satellites/pair-programming/dev.ts",
  };

  it("includes PartOf anima.service and ExecStart", () => {
    const unit = renderSatelliteSystemdUnit("pair-programming", launch);
    ...
    expect(unit).toContain("ExecStart=bun satellites/pair-programming/dev.ts");
    expect(unit).toContain("Environment=STUDIO_WORKSPACE=/home/user/project");

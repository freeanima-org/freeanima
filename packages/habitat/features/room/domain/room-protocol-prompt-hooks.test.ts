...
import { createHookContext, runSystemPromptBuild } from "@freeanima/habitat/core/hooks/cordis";
import { PROMPT_XML_TAGS, foldSystemPromptSections } from "@freeanima/habitat/core/hooks/prompt";
...
describe("registerRoomProtocolSystemPromptHook", () => {
  it("仅 room_inner 注入协议段，文案提及 room_members 与唯一用户", async () => {
    ...
    registerRoomProtocolSystemPromptHook(ctx);

    const withRoom = await runSystemPromptBuild(ctx, {
      functionNames: [],
      mode: "digital_human",
      meta: {
        model: "m",
    ...
    const roomText = foldSystemPromptSections(withRoom);
    ...
    const without = await runSystemPromptBuild(ctx, {
      functionNames: [],
      mode: "digital_human",
      meta: {
        model: "m",
    ...
    const plain = foldSystemPromptSections(without);

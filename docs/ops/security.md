...
| 浏览器扩展             | 浏览器形态入口（`packages/frontend/portal/extension`；展示名 FreeAnima；Firefox gecko `extension@freeanima.com`）；Service API Token 存 `chrome.storage.local`；主密码仅扩展内存（默认 15min）；RPC 走 background `host_permissions`。见 [`portal.md`](../modules/portal.md)、[`vault.md`](../modules/vault.md) |

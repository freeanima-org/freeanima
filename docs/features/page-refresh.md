...
| ----------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| **sync**    | Hub reconnect, tab visible again, Offline sync toast retry | Flush outbox; modules that flushed call `refreshAll` (see [offline platform](../guide/offline-platform.md))                    |
...
- **Out of scope**: Hub fan-out for every CRUD entity; React Query; turning the offline-sync toast into a page refresh control.
...
| E Console        | Ops lists                         | Load on enter                                                          | Explicit refresh (existing)               |
...
- Copy: `m.console_common_refresh` / `m.console_common_refreshing`.

...
import type { SkillOrigin } from "@freeanima/habitat/core/db/schema/entity";
...
export async function loadSkillIntoContext(skills: SkillRegistry, name: string): Promise<string> {
  ...
  const { getToolConversationId } = await import("@freeanima/habitat/core/tool/tool-context.ts");

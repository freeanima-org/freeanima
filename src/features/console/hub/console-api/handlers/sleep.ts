...
export async function getSleepSummary() {
  return consoleCtx().getSleepSummary();
}
...
export async function listPipelineStepRuns(opts?: {
  ...
  return consoleCtx().listPipelineStepRuns(opts);
}
...
export async function startSleepCycle(body?: {
  ...
  const result = await consoleCtx().startSleepCycle(
    omitUndefined({
      day: body?.day,
...
export async function startSleepPipelineStep(body: {
  ...
  const result = await consoleCtx().startSleepPipelineStep(
    omitUndefined({
      stepId: body.step_id,
      day: body.day,
      force: body.force,

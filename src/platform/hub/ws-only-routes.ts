...
import { wsOnlyMeta } from "@freeanima/shared/hub-contract";
...
export const wsOnlyHubRoutes = mergeFeatureRoutes([
  defineHubRoute({
    ...
    meta: wsOnlyMeta(),
    handler: async () => {
      throw new Error("sap.attach is handled by Hub RPC transport layer");
    },
  }),
  defineHubRoute({
    method: "sap.detach",
    ...
    meta: wsOnlyMeta(),
    handler: async () => {
      throw new Error("sap.detach is handled by Hub RPC transport layer");
    },
  }),
  defineHubRoute({

...
export const CAPABILITIES_TOOLS_RETURNS: Record<string, ToolReturnContractFields> = {
  ...
  skill_create: defineToolReturn({
    ...
    example: {
      ...
      origin: "user",
      entity_id: 1,
      message: "Skill 'my-skill' created (origin=user)",
    },
  }),
  skill_patch: defineToolReturn({
    schema: z.object({
      ok: z.literal(true),
      name: z.string(),
      message: z.string(),
    }),
    example: { ok: true, name: "my-skill", message: "Skill 'my-skill' patched" },
  }),
  skill_update: defineToolReturn({
    schema: z.object({
      ok: z.literal(true),
      name: z.string(),
      message: z.string(),
    }),
    example: { ok: true, name: "my-skill", message: "Skill 'my-skill' updated" },
  }),

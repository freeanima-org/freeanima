...
export async function probeHubHealth(
  ...
  try {
    ...
    logCaughtError("hub-rpc/probeHubHealth", err);
    return false;
  }
}
...
async function probeHubHealthDefault(): Promise<boolean> {
  try {
    ...
    logCaughtError("hub-rpc/probeHubHealthDefault", err);
    return false;
  }
}

export function useHubRpcConnectivity(enabled: boolean): {
  state: HubRpcConnectionState;
  retry: () => Promise<void>;
} {
  const [state, setState] = useState<HubRpcConnectionState>("connecting");

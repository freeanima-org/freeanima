...
**Search index (PG):** rebuildable search data lives on side table `search_documents` (not on business `entities` / `messages` rows). Docs are keyed `ent:{id}` / `msg:{id}`; jieba writes `fts_segmented` → generated `search_fts`; embeddings land on `search_documents.embedding` asynchronously. Business CRUD calls `SearchBackend.upsert` (`PgSearchIndex` by default; optional `fts.backend: pg_business_scan` scans business fields only). Limbic / autobiographical / dream narratives live as `entities` `content_block`s (with `limbic` / `narrative` / `dream` tags) under dated `diary_entry`. Jieba runs synchronously on upsert (failure → null, row still writes); embedding runs asynchronously after insert (failure logged only). Persistent jieba / embedding soft failures and memory search paths that swallow errors into empty hits also dual-Inbox notify (CST-day dedupe) — see [Bypassable soft failure](notifications.md#bypassable-soft-failure).

**Hybrid retrieval:** pipeline is retrieve (channels `fts` / `trgm`, optional `vector`) → fuse (RRF) → optional rerank → hydrate business rows. Default hybrid runs FTS and trigram in **one parallel wave**, then RRF. Auto-built FTS queries join tokens with **OR** (space-separated / jieba segments); explicit `AND`/`OR`/`NOT` still work; unquoted CJK longer than two characters uses **bigram-OR**, while quoted phrases keep full adjacency. Keyword/FTS relevance is prioritized; vector similarity is not part of retrieval by default (channel reserved; avoids low-relevance semantic neighbors). RRF here is **within one index/scope**, not across memory types. Scope uses **filters** (`resource`, `primary_component`, …), not a flat kind enum.

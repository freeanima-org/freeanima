...
describe("createRpcClient request timeout", () => {
  it(
    ...
    async () => {
      ...
      ).rejects.toBeInstanceOf(HabitatRpcTimeoutError);
    },
    { timeout: HABITAT_RPC_DEFAULT_REQUEST_TIMEOUT_MS + 2_000 },
  );
  ...
  it(
    ...
    async () => {
      ...
      expect(elapsed).toBeLessThan(HABITAT_RPC_MESSAGE_SEND_TIMEOUT_MS);
    },
    { timeout: 2_000 },
  );
});

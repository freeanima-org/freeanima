...
import { getLimbicMemory } from "@freeanima/habitat/core/db/pg/limbic-memory";
import {
  getSemanticMemory,
...
export async function buildLightSleepAutobiographyUserMessages(
  ...
  if (!blocks) {
    if (!range) {
      throw new Error(
        "buildLightSleepAutobiographyUserMessages requires precomputedBlocks or range",
      );
    }
    blocks = await collectConversationBlocks(conversationIds, range);
  }

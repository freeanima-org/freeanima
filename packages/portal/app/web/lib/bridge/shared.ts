import type { WebUiConfigJson } from "@freeanima/portal-sdk/web-ui-config";
...
export type ScopedSettingsBridge =
  import("@freeanima/portal-sdk/settings").ScopedSettingsBackend & {
    test(
      scope: import("@freeanima/portal-sdk/settings").SettingsStorageScope,
      value: unknown,
    ): Promise<unknown>;
  };
...
export async function fetchWebUiConfig(): Promise<WebUiConfigJson | null> {
  try {
    ...
    const { parseWebUiConfigJson } = await import("@freeanima/portal-sdk/web-ui-config");
